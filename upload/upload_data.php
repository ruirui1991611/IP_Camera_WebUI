...
if ($upload_file["error"]==0 && !empty($upload_file)) {
	...
	if (move_uploaded_file($_FILES["file"]["tmp_name"], $query_path)) {
		...
		if ($ret == -1) {
			...
			// 2. 返回上传失败信息，并提示用户
			$data_out = $data_out.'"上传失败,文件不符合要求(必须是sqlite数据库文件)"';
		} else {
			...
			// 2. 文件符合要求，将其拷贝至/var/www/ipc-webui/face.db ，
			//    然后删除query_path，并通过ipc-property设置database path
			$copy_path = "/var/www/ipc-webui/face.db";
			...
			// 3. 返回上传成功信息
			$data_out = $data_out.'"上传成功"';
		}
	} else {
		$data_out = $data_out.'"上传失败,移动临时文件出错"';
	}
} else {
	$data_out = $data_out.'"上传失败,网络错误"';
}
